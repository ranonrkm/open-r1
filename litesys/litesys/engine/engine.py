...
class LLM:
    ...
    attn_topk_block: int = 0):
        ...
        self.kv_cache = BatchKVManager(self.model_config, self.max_batch_size, self.max_seq_len, self.device, self.dtype, attn_local=attn_local, attn_topk=attn_topk, attn_nsample=attn_nsample, attn_topk_block=attn_topk_block)
        ...
        logger.info(f"  Attn Local      : {attn_local}")
        logger.info(f"  Attention Topk  : {attn_topk}")
        logger.info(f"  Attention Nsample: {attn_nsample}")
        logger.info(f"  Attention Topk Block: {attn_topk_block}")
