...
flash_attn_func_custom = FlashAttnFunc.apply
...
def test_fwd(Z, H, H_kv, N_CTX, HEAD_DIM, dtype=torch.bfloat16):
    ...
    ref_out = flash_attn_func(q, k, v, softmax_scale=sm_scale, causal=True)

    # custom implementation
    out = flash_attn_func_custom(q, k, v, None, True, sm_scale)
    
    # compare
    assert torch.allclose(out, ref_out, atol=1e-2, rtol=0)

if __name__ == '__main__':
    test_fwd(1, 8, 8, 1024, 128)