...
# --------------------------- public API ---------------------------
class CPSLHFlashAttention(torch.autograd.Function):
    @staticmethod
    def forward(ctx, q, k, v, q_codes, k_codes, causal, sm_scale, dropout_p, local_size, return_log_normalizer, return_total_attention, return_seed_offset):
        ...
        assert q.shape[:3] == q_codes.shape[:3], "q and q_codes should have the same shape except for the last dimension"
        assert k.shape[:3] == k_codes.shape[:3], "k and k_codes should have the same shape except for the last dimension"
        assert q_codes.shape[-1] == (k_codes.shape[-1]), "q_codes and k_codes should have the same shape"
        lsh_L = q_codes.shape[-1]
        ...
        q_codes, k_codes = maybe_contiguous(q_codes), maybe_contiguous(k_codes)
        ...
        with torch.cuda.device(device):
            ...
            assert not split_kv, "split kv is not supported yet"
            ...
            if not split_kv:
                ...
                cnts = torch.zeros((B, H, M), device=q.device, dtype=torch.int32)
                _fwd_kernel[grid](
                    q, k, v, q_codes, k_codes, sm_scale,
                    ...
                    q_codes.stride(0), q_codes.stride(1), q_codes.stride(2), q_codes.stride(3), 
                    k_codes.stride(0), k_codes.stride(1), k_codes.stride(2), k_codes.stride(3), 
        ...
        ctx.save_for_backward(q, k, v, q_codes, k_codes, o, L)
    ...
    def backward(ctx, do, *ignored):
        q, k, v, q_codes, k_codes, o, L = ctx.saved_tensors
        ...
        with torch.cuda.device(device):
            ...
            grid = (triton.cdiv(N, BLOCK_N), H, B)
            _bwd_kv_kernel[grid](
                q, k, v, q_codes, k_codes, sm_scale, do,
                dk, dv,
                L, delta,
                dropout_p,
                seed,
...
def cplsh_attention(q, k, v, q_codes, k_codes, causal=False, sm_scale=None, dropout_p=0.0, local_size=0,
    ...
    """
    return CPSLHFlashAttention.apply(q, k, v, q_codes, k_codes, causal, sm_scale, dropout_p, local_size, return_log_normalizer, return_total_attention, return_seed_offset)
...
def _fwd_kernel(
    ...
    stride_qc_z, stride_qc_h, stride_qc_m, stride_qc_l,
    stride_kc_z, stride_kc_h, stride_kc_n, stride_kc_l,
    Z, H, M, N, P_SEQ, num_groups, lsh_L: tl.constexpr, LOCAL_SIZE: tl.constexpr,
    ...
    cnts += (off_z * H + off_h) * M # cnts's shape is (B, H, M)

    Q_codes += off_z * stride_qc_z + off_h * stride_qc_h    # (M, L, K)
    K_codes += off_z * stride_kc_z + off_hk * stride_kc_h   # (N, L, K)

    offs_m_base = tl.arange(0, BLOCK_M)
    offs_m = start_m * BLOCK_M + offs_m_base
    offs_n_base = tl.arange(0, BLOCK_N)
    offs_k = tl.arange(0, BLOCK_DMODEL)
    offs_lsh_l = tl.arange(0, lsh_L)
    ...
    l_ptrs = L + offs_m
    cnts_ptrs = cnts + offs_m
    q_codes_ptrs = Q_codes + (offs_m[:, None] * stride_qc_m + offs_lsh_l[None, :] * stride_qc_l) # (BLOCK_M, lsh_L)
    ...
    if DIVISIBLE_M:
        ...
        q_codes = tl.load(q_codes_ptrs, cache_modifier=".cg")
    else:
        mask_m = offs_m < M
        q = tl.load(q_ptrs, mask=mask_m[:, None], cache_modifier=".cg")
        q_codes = tl.load(q_codes_ptrs, mask=mask_m[:, None], cache_modifier=".cg")
    ...
    k_codes_ptrs = K_codes + (offs_n_init[:, None] * stride_kc_n + offs_lsh_l[None, :] * stride_kc_l) # (BLOCK_N, lsh_l)
    for start_n in range(0, hi, BLOCK_N):
        ...
        if DIVISIBLE_N:
            ...
            k_codes = tl.load(k_codes_ptrs, cache_modifier=".cg")
        else:
            mask_n = offs_n < N
            k = tl.load(k_ptrs, mask=mask_n[None, :], cache_modifier=".cg")
            v = tl.load(v_ptrs, mask=mask_n[:, None], cache_modifier=".cg")
            k_codes = tl.load(k_codes_ptrs, mask=mask_n[:, None], cache_modifier=".cg")

        # -- compute qk ---
        s = tl.zeros([BLOCK_M, BLOCK_N], dtype=tl.float32)
        s += tl.dot(q, k)

        # -- compute hash matches ---
        # (q_codes[:, None, :] == k_codes[None, :, :]).any(dim=-1) 
        matches = q_codes[:, None, :] == k_codes[None, :, :]
        hash_mask = tl.sum(matches, 2) > 0
        
        # use hash mask and retain local
        hash_mask = hash_mask | (P_SEQ + offs_m[:, None] - LOCAL_SIZE < offs_n[None, :])
        s = tl.where(hash_mask, s, -1e6)    # float("-inf")) sometimes causes NaN probably because all values in a block are -inf
        ...
        k_codes_ptrs += BLOCK_N * stride_kc_n
    ...
    if DIVISIBLE_M:
        ...
        tl.store(cnts_ptrs, cnt, cache_modifier=".cg")
    else:
        tl.store(l_ptrs, l, mask=mask_m, cache_modifier=".cg")
        tl.store(o_ptrs, acc.to(input_dtype), mask=mask_m[:, None], cache_modifier=".cg")
        tl.store(cnts_ptrs, cnt, mask=mask_m, cache_modifier=".cg")
...
def _bwd_kv_kernel(
    ...
    Z, H, M, N, P_SEQ,
    num_groups, lsh_L: tl.constexpr, LOCAL_SIZE: tl.constexpr,
    BLOCK_M: tl.constexpr, BLOCK_DMODEL: tl.constexpr, BLOCK_N: tl.constexpr,
    ...
    # hash code offset pointers for (batch, head)
    Q_codes += off_z * stride_qc_z + off_h * stride_qc_h    
    K_codes += off_z * stride_kc_z + off_hk * stride_kc_h
    ...
    q_codes_ptrs = Q_codes + (offs_m_init[:, None] * stride_qc_m + offs_lsh_l[None, :] * stride_qc_l) # (BLOCK_M, lsh_L)
    k_codes_ptrs = K_codes + (offs_n[:, None] * stride_kc_n + offs_lsh_l[None, :] * stride_kc_l) # (BLOCK_N, lsh_L)
    ...
    if DIVISIBLE_N:
        v = tl.load(v_ptrs)
        k = tl.load(k_ptrs)
        k_codes = tl.load(k_codes_ptrs)
    else:
        mask_n = offs_n < N
        v = tl.load(v_ptrs, mask=mask_n[:, None])
        k = tl.load(k_ptrs, mask=mask_n[:, None])
        k_codes = tl.load(k_codes_ptrs, mask=mask_n[:, None])
    ...
    for start_m in range(lo, M, BLOCK_M):
        ...
        if DIVISIBLE_M:
            q = tl.load(q_ptrs)
            q_codes = tl.load(q_codes_ptrs)
        else:
            mask_m = offs_m < M
            valid_mask = mask_m[:, None] # & mask_n
            q = tl.load(q_ptrs, mask=mask_m[:, None])
            q_codes = tl.load(q_codes_ptrs, mask=mask_m[:, None])
        ...
        q_codes_ptrs += BLOCK_M * stride_qc_m
