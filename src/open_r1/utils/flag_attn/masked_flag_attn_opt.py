...
class FlashAttention(torch.autograd.Function):
    @staticmethod
    def forward(ctx, q, k, v, block_mask, causal, sm_scale, dropout_p, return_log_normalizer, return_total_attention, return_seed_offset):
        '''
        q: B H M D
        k: B H_kv M D
        v: B H_kv M D
        block_mask: B H_kv M (M // BLOCK_SIZE) # BLOCK_SIZE is typically 16 or 32, same block is shared across query heads in a group
        '''
        ...
        N_kv_block = block_mask.shape[-1]
        assert N % N_kv_block == 0, "number of keys must be a multiple of the number of kv blocks"
        kv_block_size = N // N_kv_block
        ...
        # contiguity
        q, k, v, block_mask = maybe_contiguous(q), maybe_contiguous(k), maybe_contiguous(v), maybe_contiguous(block_mask)
        ...
        with torch.cuda.device(device):
            ...
            if not split_kv:
                ...
                _fwd_kernel[grid](
                    ...
                    B, H, M, N, P_SEQ, kv_block_size, num_groups,
        ...
        ctx.save_for_backward(q, k, v, block_mask, o, L)
    ...
    def backward(ctx, do, *ignored):
        q, k, v, block_mask, o, L = ctx.saved_tensors
...
def masked_attention(q, k, v, block_mask, causal=False, sm_scale=None, dropout_p=0.0,
    ...
    """
    return FlashAttention.apply(q, k, v, block_mask, causal, sm_scale, dropout_p, return_log_normalizer, return_total_attention, return_seed_offset)
...
def _fwd_kernel(
    ...
    Z, H, M, N, P_SEQ, kv_block_size: tl.constexpr,
    num_groups,
    ...
    BLK_MASK += off_z * stride_bz + off_hk * stride_bh
    ...
    BLOCK_MASK_N = BLOCK_N // kv_block_size
    ...
    offs_blk_n_init = offs_n_base // kv_block_size
    ...
    blk_mask_ptrs = BLK_MASK + (offs_m[:, None] * stride_bm + offs_blk_n_init[None, :] * stride_bn) # (BLOCK_M, BLOCK_N)
    
    for start_n in range(0, hi, BLOCK_N):
        start_n = tl.multiple_of(start_n, BLOCK_N)
        offs_n = start_n + offs_n_base
        offs_blk_n = offs_n // kv_block_size
        ...
        if DIVISIBLE_N:
            ...
            if DIVISIBLE_M:
                blk_mask = tl.load(blk_mask_ptrs, cache_modifier=".cg")
            else:
                blk_mask = tl.load(blk_mask_ptrs, mask=mask_m[:, None], cache_modifier=".cg")
        else:
            mask_n = offs_n < N
            k = tl.load(k_ptrs, mask=mask_n[None, :], cache_modifier=".cg")
            v = tl.load(v_ptrs, mask=mask_n[:, None], cache_modifier=".cg")
            if DIVISIBLE_M:
                blk_mask = tl.load(blk_mask_ptrs, mask=mask_n[None, :], cache_modifier=".cg")
            else:
                blk_mask = tl.load(blk_mask_ptrs, mask=mask_m[:, None] & mask_n[None, :], cache_modifier=".cg")
        ...
        blk_mask_ptrs += BLOCK_MASK_N * stride_bn
...
def _bwd_kv_kernel(
    ...
    for start_m in range(lo, M, BLOCK_M):
        ...
        if DIVISIBLE_M:
            q = tl.load(q_ptrs)
            if DIVISIBLE_N:
                blk_mask = tl.load(blk_mask_ptrs, cache_modifier=".ca")
            else:
                blk_mask = tl.load(blk_mask_ptrs, mask=mask_n[None, :], cache_modifier=".ca")
        else:
            mask_m = offs_m < M
            valid_mask = mask_m[:, None] # & mask_n
            q = tl.load(q_ptrs, mask=mask_m[:, None])
            if DIVISIBLE_N:
                blk_mask = tl.load(blk_mask_ptrs, mask=mask_m[:, None], cache_modifier=".ca")
            else:
                blk_mask = tl.load(blk_mask_ptrs, mask=mask_m[:, None] & mask_n[None, :], cache_modifier=".ca")
        ...
        blk_mask_ptrs += BLOCK_M * stride_bm
